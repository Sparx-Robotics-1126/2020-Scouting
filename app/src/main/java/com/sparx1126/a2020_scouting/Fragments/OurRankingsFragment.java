...
public class OurRankingsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        Map<Integer, Float> dataOuter = OurRankingData.getBallsScoredOnOuterAve();
        Map<Integer, Float> dataBottem = OurRankingData.getBallsScoredOnBottomAve();
        ...
        if (data.size() == 0) {
            ...
            Map<Integer, Float> tmpdataouter = new HashMap<>();
            Map<Integer, Float> tmpdatabottom = new HashMap<>();
            tmpdataouter.putAll(dataOuter);
            tmpdatabottom.putAll(dataBottem);
            tmpdata.putAll(data);
            Log.d(TAG, HEADER + "DATA SIZE " + data.size());
            for (int i = 0; i < data.size(); i++) {
                Pair<Integer, Float> highest = getHighest(tmpdata);
                Pair<Integer, Float> highestouter = getHighest(tmpdataouter);
                Pair<Integer, Float> highestbottom = getHighest(tmpdatabottom);
                ...
                details.setText("Balls Scored on Bottom : " + String.valueOf(highestbottom.second) + " , Balls Scored On Outer : " +
                        String.valueOf(highestouter.second) + " , Balls Scored On Inner : " + String .valueOf(highest.second));
